from __future__ import annotations
from seedemu import *
from ipaddress import IPv4Address, IPv4Network


OpenVpnFileTemplates: Dict[str, str] = {}

OpenVpnFileTemplates['ovpn_key'] = '''\
-----BEGIN PRIVATE KEY-----

PF9Izl/W0GZPOJu6oPEm7uHR
-----END PRIVATE KEY-----
'''

OpenVpnFileTemplates['ovpn_ca'] = '''\
-----BEGIN CERTIFICATE-----

vgv5PvXzjCVY71b1ZrjHo0SUDGRC
-----END CERTIFICATE-----
'''

OpenVpnFileTemplates['ovpn_cert'] = '''\
-----BEGIN CERTIFICATE-----

MBV5ibZ7
-----END CERTIFICATE-----
'''

OpenVpnFileTemplates['ovpn_server_config'] = '''\
mode server
tls-server
verb 3
<key>
{key}
</key>
<ca>
{ca}
</ca>
<cert>
{cert}
</cert>
<dh>
-----BEGIN DH PARAMETERS-----

-----END DH PARAMETERS-----
</dh>
<tls-auth>
-----BEGIN OpenVPN Static key V1-----
2f7d9e47a8f9843c66821d9fcf9f9b5d
5d04e1d401f855593f0348e3b78cf746
4a2fb78a5c966b0bd601f4886d07ce2e
927c6e60359a3eff91df118da1260604
d8902cf6447e4568e3525962aa34d878
10c848b92a658be9f8fe948e1cc060b1
09fd6efe2598fc4be0faae0dfb62dadc
860c62c126a1c8af67d00b1e26342bbc
889700c790d0159de2e27eb005c08e49
0a8db7910241031a86b8feb23317e6c0
d47d528ee78b190baee442359b4ed243
9c1d99e724e7156de525ddd94145b588
e466272c2ab0b8d39b3da74d5794d6d4
00aff34bfa61f78ffb535d4b27c818ea
4e4b8a9efc6927b18a8e784d9236c3e2
c74e92120ac39909f3fe5da198f4e661
-----END OpenVPN Static key V1-----
</tls-auth>
key-direction 0
keepalive 10 60
persist-key
duplicate-cn
proto udp
port 1194
status /tmp/openvpn-status.log
user nobody
group nogroup
client-to-client
dev tap0
ifconfig-pool {addressStart} {addressEnd} {addressMask}
'''

OpenVpnFileTemplates['ovpn_startup_script'] = '''\
#!/bin/bash
[ ! -d /dev/net ] && mkdir /dev/net
mknod /dev/net/tun c 10 200 > /dev/null 2>&1
openvpn --config /ovpn-server.conf &
while ! ip li sh tap0 > /dev/null 2>&1; do sleep 1; done
ip li set tap0 up
addr="`ip -br ad sh "$1" | awk '{ print $3 }'`"
ip addr fl "$1"
brctl addbr br0
brctl addif br0 "$1"
brctl addif br0 tap0
ip addr add "$addr" dev br0
ip li set br0 up
'''


class VPNServer(Server):
    """!
    @brief The VPN server using OpenVPN
    """

    def __init__(self, naddrs: int = 8, ovpnCa: str = None, ovpnCert: str = None, ovpnKey: str = None):
        """!
        @brief VPNServer Constructor.

        If you do not set ca/cert/key, the builtin ones will be used. 
        To connect, use the client configuration in the misc/ folder. 

        @param naddrs number of IP addresses to assign to client pool.
        @param ovpnCa (optional) CA to use for openvpn.
        @param ovpnCert (optional) server certificate to use for openvpn.
        @param ovpnKey (optional) server key to use for openvpn.
        """
        super().__init__()

        self.__naddrs = naddrs
        self.__ovpn_ca = ovpnCa
        self.__ovpn_cert = ovpnCert
        self.__ovpn_key = ovpnKey
    
    def configure(self, node: Node, vpn: VPNService, emulator:Emulator):
        """!
        @brief configure the node, if necessary
        """
        return 

    def install(self, node:Node, vpn: VPNService, emulator: Emulator):
        """!
        @brief Install the service
        """

        node.addSoftware('openvpn')
        node.addSoftware('bridge-utils')

        addrstart = "<address-start>"
        addrend   = "<address-end>"
        addrmask  = "<netmask>"

        node.setFile('/ovpn-server.conf', OpenVpnFileTemplates['ovpn_server_config'].format(
            addressStart = addrstart,
            addressEnd = addrend,
            addressMask = addrmask,
            key = self.__ovpn_key if self.__ovpn_key != None else OpenVpnFileTemplates['ovpn_key'],
            ca = self.__ovpn_ca if self.__ovpn_ca != None else OpenVpnFileTemplates['ovpn_ca'],
            cert = self.__ovpn_cert if self.__ovpn_cert != None else OpenVpnFileTemplates['ovpn_cert']
        ))

        node.setFile('/ovpn_startup', OpenVpnFileTemplates['ovpn_startup_script'])
        node.appendStartCommand('chmod +x /ovpn_startup')

        # note: ovpn_startup will invoke interface_setup, and replace interface_setup script with a dummy. 
        #node.appendStartCommand('/ovpn_startup {}'.format(netObject.getName()))
        #node.appendStartCommand('ip route add default via {} dev {}'.format(brNet.getPrefix()[1], brNet.getName()))


    
    def print(self, indent: int) -> str:
        out = ' ' * indent
        out += 'VPN server object.\n'

        return out


class VPNService(Service):
    """!
    @brief The virtual private network service class.
    """

    def __init__(self):
        """!
        @brief VPNService constructor
        """
        super().__init__()
        self.addDependency('Base', False, False)

    def _createServer(self) -> Server:
        return VPNServer()

    def getName(self) -> str:
        return 'VPNService'

    def print(self, indent: int) -> str:
        out = ' ' * indent
        out += 'VPNServiceLayer\n'

        return out
